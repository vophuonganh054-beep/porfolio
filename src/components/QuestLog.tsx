...
import { getBilingualQuestItems, TRANSLATIONS } from '../utils/translations';

export const QuestLog: React.FC = () => {
  const { language, isVi } = useLanguage();
  const t = TRANSLATIONS[language].quests;
  const questItems = getBilingualQuestItems(isVi);
        ...
        {questItems.map((quest) => (
          ...
          >
            ...
            <div className="mt-4 pt-3 border-t-2 border-[#5a3696] bg-[#231b34] -mx-5 -mb-5 p-3 flex items-center justify-between">
              <span className="font-['Space_Mono'] text-xs text-[#eaddff] font-bold">{t.rewardLabel}</span>
